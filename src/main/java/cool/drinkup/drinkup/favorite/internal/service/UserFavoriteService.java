...
public class UserFavoriteService {
    ...
    public UserFavoriteService(UserFavoriteRepository favoriteRepository,
                                List<FavoriteObjectLoader<?>> loaders) {
    ...
    public Page<UserFavoriteDTO> getUserFavoritesWithDetails(Long userId, FavoriteType objectType, Pageable pageable) {
        ...
        // 2. 按类型分组
        Map<FavoriteType, List<UserFavorite>> groupedFavorites = favoritePage.getContent().stream()
                .collect(Collectors.groupingBy(UserFavorite::getObjectType));
        ...
        for (Map.Entry<FavoriteType, List<UserFavorite>> entry : groupedFavorites.entrySet()) {
            ...
            List<Long> objectIds = favorites.stream()
                    .map(UserFavorite::getObjectId)
        ...
        // 5. 按原始顺序排序
        List<Long> originalIds = favoritePage.getContent().stream()
                .map(UserFavorite::getId)
                .collect(Collectors.toList());
    ...
    public Map<Long, Boolean> checkFavoriteStatus(Long userId, FavoriteType objectType, List<Long> objectIds) {
        List<UserFavorite> favorites = favoriteRepository.findByUserIdAndObjectTypeAndObjectIdIn(userId, objectType, objectIds);
