...
public interface MaterialRepository extends JpaRepository<Material, Long> {
     ...
     * 根据材料名称搜索（支持中英文）
     */
    @Query("SELECT m FROM Material m WHERE m.isActive = true AND (m.name LIKE %:keyword% OR"
            + " m.nameEn LIKE %:keyword%) ORDER BY m.sortOrder ASC")
    List<Material> searchByName(@Param("keyword") String keyword);

    /**
     * 根据分类ID列表获取材料
     */
    @Query("SELECT m FROM Material m WHERE m.categoryId IN :categoryIds AND m.isActive = true"
            + " ORDER BY m.categoryId ASC, m.sortOrder ASC")
