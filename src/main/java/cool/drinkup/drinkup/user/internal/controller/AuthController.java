...
public class AuthController {
    ...
    public ResponseEntity<CommonResp<String>> sendVerificationCode(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(CommonResp.error("验证码发送失败: " + e.getMessage()));
        }
    }
    ...
    @LogRecord(
            type = UserEvent.USER,
            subType = "{{#_ret.body.data.isNewUser ?"
                    + " T(cool.drinkup.drinkup.common.log.event.UserEvent$BehaviorEvent).REGISTER"
                    + " : T(cool.drinkup.drinkup.common.log.event.UserEvent$BehaviorEvent).LOGIN}}",
    ...
    public ResponseEntity<CommonResp<UserLoginResp>> login(
            @Parameter(description = "登录信息，支持手机号验证码登录、Google 登录和 Apple 登录") @Valid @RequestBody
        ...
        try {
            ...
            if (!loginStrategy.validateCredentials(loginRequest)) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(CommonResp.error("登录凭据验证失败"));
            }
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(CommonResp.error("登录失败: " + e.getMessage()));
        }
    }

    @LogRecord(type = UserEvent.USER, subType = UserEvent.BehaviorEvent.LOGOUT, bizNo = "null", success = "用户登出成功")
