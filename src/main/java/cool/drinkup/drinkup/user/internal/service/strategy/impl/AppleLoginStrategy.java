...
public class AppleLoginStrategy implements LoginStrategy {
        ...
        try {
            ...
            if (StringUtils.hasText(email) && (isPrivateEmail == null || !isPrivateEmail)) {
                ...
                if (userByEmail.isPresent()) {
                    ...
                    // 为现有用户绑定新的Apple账号
                    userOAuthService.createOAuthBinding(
                            user, oauthId, OAuthTypeEnum.APPLE, email, extractUsernameFromEmail(email), null);
     ...
     * 创建新用户并绑定Apple账号
     */
    private User createNewUserWithAppleBinding(
            String oauthId, String email, Boolean emailVerified, Boolean isPrivateEmail) {
        ...
        userOAuthService.createOAuthBinding(savedUser, oauthId, OAuthTypeEnum.APPLE, email, oauthUsername, null);
